...
@media_bp.route('/addMedia', methods=['POST'])
...
def add_media():
    ...
    Request Body:
        url: S3/CloudFront URL of media
        contentType: MIME type of media
        width: Width of media (for images/videos)
        height: Height of media (for images/videos)
        blurHash: Optional blurhash for images
        objectKey: S3 object key
        source: Media source (INTERNET/USER_AVATAR/VOLCENGINE)
        
    Returns:
        JSON response with created media data
    """
    try:
        ...
        if not url:
            return create_response(code=200, message="url is required"), 200
        
        if not content_type:
            return create_response(code=200, message="contentType is required"), 200
        
        # Determine media type from content type
        media_type = 'VIDEO' if content_type.startswith('video/') else 'IMAGE'
        

        result = MediaService.add_media(
        ...
        return create_response(code=200, message="Failed to add media"), 200
